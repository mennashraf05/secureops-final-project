...
export default function SecurityCenter() {
 ...
 const [deletingRiskKey, setDeletingRiskKey] = useState<string | null>(null);
 ...
 async function handleDeleteRiskEvents(score: UserRiskScore) {
  const riskKey = score.user_id === null ? 'system' : String(score.user_id);
  const label = score.user_name || (score.user_id === null ? 'System / Unknown' : `User #${score.user_id}`);
  if (!window.confirm(`Delete risk events for ${label}? This removes the related audit-log events from risk scoring.`)) return;

  setError('');
  setMessage('');
  setDeletingRiskKey(riskKey);
  try {
   const result = await deleteUserRiskEvents(score.user_id === null ? 'system' : score.user_id);
   setMessage(`Deleted ${result.deleted_count} risk event${result.deleted_count === 1 ? '' : 's'} for ${label}.`);
   await loadOverview();
  } catch (err) {
 ...
 const userRiskRows = userRiskScores.map((score) => {
   ...
   return [
    ...
    <Button variant="danger" onClick={() => void handleDeleteRiskEvents(score)} disabled={deletingRiskKey === riskKey}>
     <Trash2 size={16} className="mr-2 inline"/>{deletingRiskKey === riskKey ? 'Deleting...' : 'Delete'}
    </Button>,
   ];
  });
