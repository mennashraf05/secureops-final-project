...
import { ShieldAlert, Swords, Loader2, CheckCircle2, XCircle, RefreshCw, Clock, ChevronDown, ChevronUp, Terminal } from 'lucide-react';
...
interface HistoryItem {
  ...
  mitigated: boolean;
  findings: string;
  logs: string[]; // Maps to backend Column(JSON)
  created_at: string;
}
          ...
          <SectionCard title="Command Center" dark>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-3">
                <Button variant="primary" onClick={() => console.log('Bulk run')}>Run All Scenarios</Button>
                        ...
                        <>
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${run.mitigated
                                  ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-600/20'
                                  : 'bg-rose-50 text-rose-700 ring-1 ring-rose-600/20'
                                }`}>
                                {run.mitigated ? 'Mitigated / Safe' : 'Bypassed / Defenceless'}
                              </span>
                            </td>
                              ...
                              <td colSpan={5} className="px-6 py-4">
                                <div className="flex flex-col gap-3 rounded-lg border border-slate-200/80 bg-slate-900 p-4 font-mono text-xs text-slate-200 shadow-inner">
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <p className={`${run.mitigated ? 'text-emerald-400' : 'text-rose-400'} italic whitespace-normal`}>
                                      {run.findings || "No findings recorded for this simulation loop run."}
                                    </p>
                                  </div>
                                  ...
                                  <div className="space-y-1 mt-2">
                                    ...
                                    {Array.isArray(run.logs) && run.logs.length > 0 ? (
                                      ...
                                      <span className="text-slate-500 italic">No structured step-by-step logs parsed.</span>
                                    )}
                                  </div>
                                </div>
                              </td>
