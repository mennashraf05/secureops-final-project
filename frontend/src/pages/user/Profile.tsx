    ...
    <section className="grid gap-6 xl:grid-cols-2">
      <SectionCard title="Profile Information" subtitle={`Role: ${user?.role ?? 'User'}`}>
        <div className="grid gap-3">
          <input className="rounded-2xl bg-slate-100 px-4 py-3" defaultValue={user?.name ?? ''}/>
          <input className="rounded-2xl bg-slate-100 px-4 py-3" defaultValue={user?.email ?? ''}/>
          <Button>Save Changes</Button>
        </div>
      </SectionCard>
      <SectionCard title="Password Change" subtitle="Validation rules included">
        <div className="grid gap-3">
          <input className="rounded-2xl bg-slate-100 px-4 py-3" placeholder="Current password"/>
          <input className="rounded-2xl bg-slate-100 px-4 py-3" placeholder="New password"/>
